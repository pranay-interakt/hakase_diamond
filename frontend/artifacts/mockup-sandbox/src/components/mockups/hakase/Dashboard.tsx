...
import {
  ...
  TrendingUp, FlaskConical, ChevronRight, X, Download, Share2,
...
const studies = [
  ...
  { id: "MERI-2022-003", title: "Bispecific Ab in DLBCL (Completed)", phase: "Phase II", indication: "Hematology", sites: 12, enrolled: 180, target: 180, status: "Completed", months: 0, countries: 5, team: ["RK","AM"] },
];
              ...
              <div key={s.id} onClick={() => onNav("analytics")} className="flex items-center gap-4 px-5 py-3 hover:bg-slate-50 cursor-pointer group transition-colors">
                ...
                <div className="flex items-center gap-5 text-xs text-slate-500 flex-shrink-0">
                  <div className="text-center">
                    <div className="font-semibold text-slate-700">{s.phase}</div>
                  </div>
                  <div className="w-28">
                  ...
                  <div className="text-slate-400">{s.months > 0 ? `${s.months}mo left` : "Complete"}</div>
                </div>
              </div>
            ))}
          </div>
...
function StudiesScreen({ onNav }: { onNav: (s: Screen) => void }) {
  ...
  const filters = ["All", "Active", "At Risk", "Critical", "Completed"];
  const shown = filter === "All" ? studies : studies.filter(s => s.status === filter || (filter === "Active" && (s.status === "On Track" || s.status === "Recruiting")));
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 p-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {shown.map((s) => (
          <div key={s.id} className={`bg-white rounded-xl border border-slate-200 p-4 flex flex-col gap-3 hover:shadow-md hover:border-slate-300 transition-all cursor-pointer ${s.status === "Completed" ? "opacity-70" : ""}`} onClick={() => onNav("analytics")}>
            ...
            <div className="flex items-center justify-between pt-1 border-t border-slate-100">
              ...
              <div className="flex items-center gap-2">
                <button className="text-xs text-[#0EA5E9] hover:underline font-medium">Analytics</button>
                <button className="text-xs text-slate-400 hover:text-slate-600">Protocol</button>
              </div>
            </div>
          </div>
        ))}
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            <div className="border-t border-slate-100 pt-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="bg-slate-50 rounded-lg p-2"><p className="text-slate-400">Active Competing Trials</p><p className="font-semibold text-slate-700">12</p></div>
              </div>
            </div>
          </div>
        </div>
              ...
              {sites.map((s, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-[#0EA5E9] rounded-full" style={{ width: `${s.score}%` }}></div></div>
                      <span className="font-semibold text-slate-700">{s.score}</span>
                    </div>
                  </td>
                  <td className="px-4 py-2.5"><Badge variant="outline" className={`text-xs ${s.exp === "High" ? "border-emerald-200 text-emerald-700" : "border-amber-200 text-amber-700"}`}>{s.exp}</Badge></td>
                  <td className="px-4 py-2.5 text-slate-600">{s.rate}</td>
                  <td className="px-4 py-2.5"><Button variant="outline" size="sm" className="text-xs">Add Site</Button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function ExplorerScreen() {
  ...
  const trials = [
    ...
    { nct: "NCT04387942", title: "Tislelizumab vs. Docetaxel in Pre-Treated NSCLC", sponsor: "BeiGene", phase: "Phase III", status: "Active, not recruiting", enrolled: 805, brief: "RATIONALE-303 — global study of tislelizumab in 2L setting. PD-L1 unselected. Enrollment completed; awaiting final OS analysis.", match: false },
  ];
    ...
    <div className="flex-1 overflow-hidden flex flex-col bg-slate-50">
      <div className="p-5 border-b border-slate-200 bg-white">
        ...
        <div className="flex items-center gap-2 mt-3">
          ...
          <button className="text-xs text-slate-400 hover:text-slate-600 flex items-center gap-1 px-2 py-1 border border-dashed border-slate-300 rounded-full"><Plus className="w-3 h-3" /> Add Filter</button>
...
function DrafterScreen() {
  const sections = [
    { title: "1. Study Objectives", content: "The primary objective of this study is to evaluate the efficacy and safety of MEK inhibitor XR-442 in combination with anti-PD-L1 therapy in patients with advanced, unresectable or metastatic melanoma who have received no prior systemic therapy.", open: true },
    { title: "2. Study Design", content: "This is a Phase I/II, open-label, multicenter, dose-escalation and dose-expansion study. Phase I will determine the maximum tolerated dose (MTD) and recommended Phase II dose (RP2D). Phase II will evaluate preliminary efficacy at the RP2D.", open: true },
    { title: "3. Eligibility Criteria", content: "", open: false },
    { title: "4. Study Endpoints", content: "", open: false },
    { title: "5. Safety Monitoring", content: "", open: false },
    { title: "6. Statistical Considerations", content: "", open: false },
  ];
  ...
  return (
    <div className="flex-1 overflow-hidden flex bg-slate-50">
      <div className="w-5/12 border-r border-slate-200 bg-white overflow-y-auto p-5">
        ...
        <div className="space-y-3">
          {[
            { label: "Link to Study", type: "select", value: "MERI-2025-011 · Melanoma" },
            { label: "Therapeutic Area", type: "select", value: "Oncology — Melanoma" },
            { label: "Intervention", type: "text", value: "XR-442 (MEK inhibitor) + Anti-PD-L1" },
            { label: "Trial Phase", type: "phase" },
            { label: "Target Population", type: "textarea", value: "Adults ≥18 years with histologically confirmed unresectable or metastatic melanoma, ECOG PS 0–1, no prior systemic therapy for advanced disease." },
            { label: "Primary Endpoint", type: "text", value: "Objective Response Rate (ORR) per RECIST v1.1" },
            { label: "Estimated Enrollment", type: "number", value: "45" },
            { label: "Study Duration", type: "duration", value: "36 months" },
          ].map((field, i) => (
            <div key={i}>
              ...
              {field.type === "phase" ? (
                ...
                <textarea className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2 resize-none focus:outline-none focus:border-[#0EA5E9] text-slate-700" rows={3} defaultValue={field.value} readOnly />
              ) : (
      ...
      <div className="flex-1 overflow-y-auto p-5">
        <div className="bg-white rounded-xl border border-slate-200">
          ...
          <div className="divide-y divide-slate-100">
            {sections.map((s, i) => (
              <div key={i}>
                <button onClick={() => { const n = new Set(openSections); n.has(i) ? n.delete(i) : n.add(i); setOpenSections(n); }} className="w-full flex items-center justify-between px-5 py-3 hover:bg-slate-50 transition-colors text-left">
                  <span className="text-sm font-semibold text-slate-800">{s.title}</span>
                  <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${openSections.has(i) ? "rotate-180" : ""}`} />
                </button>
            ...
            {kols.map((kol, i) => (
              <div key={i} onClick={() => setSelected(i)} className={`bg-white rounded-xl border p-4 cursor-pointer hover:shadow-md transition-all ${selected === i ? "border-[#0EA5E9] shadow-md" : "border-slate-200"}`}>
                ...
                <div className="flex items-center gap-3 text-xs text-slate-500 mb-3">
                  <span>{kol.pubs} publications</span>
                ...
                <div>
                  ...
                  <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full rounded-full bg-[#0EA5E9]" style={{ width: `${kol.score}%` }}></div></div>
                </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button className="bg-[#0EA5E9] hover:bg-[#0284C7] text-white flex items-center gap-2"><Plus className="w-4 h-4" /> Invite Member</Button>
      </div>
              ...
              <tr key={i} className="hover:bg-slate-50 transition-colors">
                <td className="px-5 py-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div><p className="font-medium text-slate-800">{m.name}</p><p className="text-xs text-slate-400">{m.last}</p></div>
                  </div>
                </td>
                ...
                <td className="px-4 py-3"><Badge className="bg-emerald-100 text-emerald-700 text-xs">{m.status}</Badge></td>
                <td className="px-4 py-3">
          ...
          {pending.map((p, i) => (
            <div key={i} className="flex items-center justify-between px-5 py-3">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-sm font-medium text-slate-700">{p.email}</p><p className="text-xs text-slate-400">{p.role} · Invited {p.invited}</p></div>
              </div>
              <div className="flex items-center gap-2">
